...
export function Work() {
  ...
  return (
    <section id="work" className="mx-auto max-w-6xl px-4 py-16 sm:py-24">
      <SectionHeader n="01" eyebrow={c.work.eyebrow} title={c.work.title} note="2024 — present" />
    ...
    <div className="grid gap-5 border-b border-white/10 py-8 transition-colors group-hover:border-white/25 md:grid-cols-[auto_1fr_auto] md:gap-8 md:py-10">
      ...
      <div>
        <h3 className="flex items-center gap-3 font-display text-3xl font-semibold leading-none tracking-tight transition-colors group-hover:text-accent sm:text-4xl md:text-5xl">
          {project.name}
          {project.href && (
            <ArrowUpRight className="h-6 w-6 shrink-0 -translate-y-1 text-muted transition-all group-hover:translate-x-1 group-hover:-translate-y-2 group-hover:text-accent" />
          )}
        </h3>
        <p className="mt-3 font-mono text-xs uppercase tracking-tight text-muted">
          {project.kind}
        </p>
      ...
      <div className="flex shrink-0 flex-col gap-2 md:items-end md:pt-3 md:text-right">
        <span className="font-mono text-xs uppercase tracking-tight text-muted">{project.year}</span>
        <span className="text-sm text-ink/75">{project.role}</span>
        {project.hrefLabel && (
          <span className="font-mono text-xs text-accent">{project.hrefLabel}</span>
        )}
      </div>
    </div>
  );
