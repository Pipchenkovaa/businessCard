    ...
    <section id="experience" className="mx-auto max-w-6xl px-4 py-16 sm:py-24">
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-2 lg:gap-12">
        ...
        <div className="flex flex-col">
          ...
          <ol className="relative mt-6 ml-[6px] border-l border-line pl-7">
            {eduTimeline.map((ed, i) => {
              const isLast = i === eduTimeline.length - 1;
              return (
                <Reveal as="li" key={ed.title} delay={i * 0.1} className="relative pb-12 last:pb-0">
                  {/* node sitting on the line */}
                  <span
                    className={
                      "absolute -left-[34px] top-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full " +
                      (isLast
                        ? "bg-accent"
